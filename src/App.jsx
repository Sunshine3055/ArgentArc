...
import NewMemberHub from "./views/NewMemberHub";
import SmdBaseView from "./views/SmdBaseView";
...
export default function CaseOperationsCenter() {
  ...
  if (!userEmail) return <AuthPanel onAuthSuccess={handleAuthSuccess} createClient={createClient} />;
  ...
  return (
    <AppShell activeSection={activeSection} setActiveSection={setActiveSection} syncMode={syncMode} syncStatus={syncStatus} onSync={handleSync} userEmail={userEmail} onLogout={handleLogout}>
      ...
      {activeSection === "members" && <NewMemberHub members={members} setMembers={setMembers} setSmdBase={setSmdBase} syncClient={activeSyncClient} ownerEmail={userEmail} setSyncStatus={setSyncStatus} />}
      {activeSection === "smd" && <SmdBaseView smdBase={smdBase} setSmdBase={setSmdBase} syncClient={activeSyncClient} setSyncStatus={setSyncStatus} />}
      {activeSection === "life" && <CasesView title="Life Insurance Case Management" seedType="Life Insurance" cases={cases} setCases={setCases} syncClient={activeSyncClient} ownerEmail={userEmail} setSyncStatus={setSyncStatus} />}
      {activeSection === "annuity" && <CasesView title="Annuity Case Management" seedType="Annuity" cases={cases} setCases={setCases} syncClient={activeSyncClient} ownerEmail={userEmail} setSyncStatus={setSyncStatus} />}
      {activeSection === "training" && <TrainingView training={training} setTraining={setTraining} syncClient={activeSyncClient} ownerEmail={userEmail} setSyncStatus={setSyncStatus} />}
    </AppShell>
  );
}
