      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[0.75fr_1.25fr]">
        ...
        <Card className="rounded-3xl border-slate-200 shadow-sm">
          ...
          <CardContent className="space-y-4">
            {cases
              .filter((c) => c.status === "Urgent" || c.status === "Pending")
              .slice(0, 4)
              .map((item) => (
                <div key={item.id} className="rounded-2xl border border-slate-200 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 font-mono text-xs text-slate-500">{item.case_ref || "—"}</div>
                    </div>
            ...
            {cases.filter((c) => c.status === "Urgent" || c.status === "Pending").length === 0 && (
              <p className="text-sm text-slate-400">No urgent or pending cases.</p>
            )}
            <Separator />
